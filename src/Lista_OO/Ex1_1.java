package Lista_OO;

public class Ex1_1 {
    /*
     * Exercício 1.1. Descreva em no máximo 200 palavras o que é um automóvel e o que ele faz. Liste os substantivos e verbos separadamente. Cada substantivo corresponde a um objeto que precisará ser construído para implementar um sistema, nesse caso, um carro. Selecione 5 dos objetos que você listou e, para cada um, liste vários atributos e comportamentos. Descreva brevemente como esses objetos interagem entre si e com outros objetos na sua descrição. Estes passos que você seguiu são típicos do projeto orientado a objetos.
     
     * Um automóvel é um veículo motorizado usado para pessoas e/ou cargas;
     * 
     * Substantivos:
     * 
     * Motor
     * Rodas
     * Freios
     * Suspensão
     * Direção
     * Combustível
     * Assentos
     * Painel
     * 
     * Verbos:
     * 
     * Acelerar
     * Frear
     * Virar
     * Abastecer
     * Ligado/desligado
     * 
     * Atributos e comportamentos:
     * 
     * Motor: cilindrada, potência, torque, consumo, velocidade máxima, ruído
     * Transmissão: manual/automática, número de marchas
     * Rodas: tamanho, pressão dos pneus, material, rolamentos
     * Suspensão: tipo, amortecedores, molas, distância ao solo
     * Combustível: tipo, capacidade
     * 
     * Interligação entre objetos:
     * O motor está ligado à transmissão, que faz as rodas girarem.
     * A suspensão é responsável por manter as rodas em contato com o solo e
     * absorver as irregularidades do terreno.O tanque de combustível armazena o
     * combustível necessário para o motor.
     * 
     */
}